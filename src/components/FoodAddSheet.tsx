...
const primaryTabs: SheetTab[] = ['recent', 'common', 'templates', 'mine']
...
export function FoodAddSheet({ open, date, logs, defaultMeal, initialTab = 'recent', initialTemplateId, initialFoodId, details, templates, foods, online, aiEnabled, metadata, onEnableAI, onAIRun, onCommitMetadata, onApply, onClose }: {
  ...
  useEffect(() => {
    if (open && !wasOpen.current) {
      ...
      setDraftEntries(initialTemplate ? [{ draftId: crypto.randomUUID(), meal: defaultMeal, line: templateMealLine(initialTemplate), source: 'template', sourceId: `template:${initialTemplate.id}`, selectionCount: 1 }]
        : initialFood ? [{ draftId: crypto.randomUUID(), meal: defaultMeal, line: customFoodMealLine(initialFood), source: 'mine', sourceId: `mine:${initialFood.id}`, selectionCount: 1 }]
          : [])
    }
  ...
  useEffect(() => {
    ...
    const onPopState = () => {
      ...
      if (savingRef.current) pushGuard()
      else if (draftEntriesRef.current.length) {
  ...
  useEffect(() => {
    ...
    const siblings = [...sheet.children].filter((child) => child !== dialog) as HTMLElement[]
    ...
    window.requestAnimationFrame(() => dialog.querySelector<HTMLElement>('button:not([disabled])')?.focus({ preventScroll: true }))
  ...
  useEffect(() => {
    if (!saving) return
    const sheet = sheetRef.current
    const lock = sheet?.querySelector<HTMLElement>('.v6-saving-lock')
    if (!sheet || !lock) return
    const siblings = [...sheet.children].filter((child) => child !== lock) as HTMLElement[]
    const previous = siblings.map((element) => ({ element, inert: element.hasAttribute('inert'), ariaHidden: element.getAttribute('aria-hidden') }))
    siblings.forEach((element) => { element.setAttribute('inert', ''); element.setAttribute('aria-hidden', 'true') })
    return () => previous.forEach(({ element, inert, ariaHidden }) => {
      if (!inert) element.removeAttribute('inert')
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const focusRoot = confirmDiscard ? discardDialogRef.current : sheetRef.current
      const focusable = [...(focusRoot?.querySelectorAll<HTMLElement>('button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [href], [tabindex]:not([tabindex="-1"])') ?? [])]
  ...
  const addDraft = (line: MealLine, source: DraftFoodEntry['source'], targetMeal = meal, sourceId?: string) => {
    if (savingRef.current) return
    setDraftEntries((entries) => mergeDraftFoodEntry(entries, {
      draftId: crypto.randomUUID(), meal: targetMeal, line, source, sourceId, selectionCount: 1
    }))
    setConfirmDiscard(false)
    setSaveError('')
  }
  ...
  const addProviderDraft = (line: MealLine, candidate: FoodCandidate) => {
    ...
    setDraftEntries((entries) => [...entries, { draftId, meal, line, source: 'provider', sourceId: `provider:${candidate.source}:${candidate.sourceId}`, selectionCount: 1 }])
  ...
  const submitManual = () => {
    if (savingRef.current || !manual.name.trim() || manual.kcal === '') return
  ...
  const searchResults = normalizedQuery ? [
    ...
    ...ingredients.filter((item) => matches(item.line.label)).map((item) => ({ id: `common:${item.id}`, label: item.line.label, note: `常用 · ${item.defaultAmount} ${item.line.unit}`, add: () => addDraft(ingredientMealLine(item, item.defaultAmount), 'common', meal, `common:${item.id}`) })),
  ...
  const dateTitle = date < localDateString()
    ? `補記 ${new Intl.DateTimeFormat('zh-TW', { month: 'numeric', day: 'numeric' }).format(parseLocalDate(date))}`
    : '今天'

  const selectedCount = (sourceId: string) => draftEntries
    .filter((entry) => entry.sourceId === sourceId)
    .reduce((sum, entry) => sum + (entry.selectionCount ?? 1), 0)
  const pickerButton = (id: string, label: string, note: string, action: () => void) => {
    const count = selectedCount(id)
    return <button type="button" className={count ? 'selected' : ''} key={id} onClick={action}><span><strong>{label}</strong><small>{note}</small></span>{count ? <b>已加 ×{count}</b> : <Plus aria-hidden="true" />}</button>
  }
  ...
  return createPortal(<div className="food-sheet-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) requestClose() }}>
    <section ref={sheetRef} className="food-sheet" role="dialog" aria-modal="true" aria-labelledby="food-sheet-title" aria-busy={saving}>
      ...
      {draftExpanded && draftEntries.length > 0 && <section className="draft-entry-list v6-draft-panel" aria-label="本次新增草稿"><div className="v6-draft-panel-heading"><h3>本次草稿</h3><button type="button" onClick={() => setDraftExpanded(false)}>收合</button></div>{draftEntries.map((entry) => <div key={entry.draftId}><span><strong>{entry.line.label}</strong><small>{mealLabels[entry.meal]}{entry.source === 'provider' ? ' · 已確認來源' : ''}</small></span><label><input aria-label={`${entry.line.label}草稿份量`} type="number" min="0" step={entry.line.unit === '份' || entry.line.unit === '顆' ? 1 : 5} value={entry.line.amount} onChange={(event) => setDraftEntries((items) => items.map((item) => item.draftId === entry.draftId ? { ...item, line: { ...item.line, amount: Math.max(0, Number(event.target.value)) } } : item))} /><i>{entry.line.portionLabel ?? entry.line.unit}</i></label><button type="button" aria-label={`移除${entry.line.label}`} onClick={() => { setDraftEntries((items) => items.filter((item) => item.draftId !== entry.draftId)); setPendingMetadata((items) => { const next = { ...items }; delete next[entry.draftId]; return next }) }}><Trash2 /></button></div>)}</section>}
      {confirmDiscard && <div ref={discardDialogRef} className="discard-draft-prompt" role="alertdialog" aria-modal="true" aria-labelledby="discard-draft-title" aria-describedby="discard-draft-description"><strong id="discard-draft-title">尚有 {draftEntries.length} 項未儲存</strong><p id="discard-draft-description">捨棄後，本次草稿不會加入今日餐點。</p><div><button type="button" disabled={saving} onClick={() => setConfirmDiscard(false)}>繼續編輯</button><button type="button" disabled={saving} className="danger-text" onClick={() => { if (savingRef.current) return; setDraftEntries([]); setPendingMetadata({}); onClose() }}>捨棄本次輸入</button></div></div>}
      <footer className="draft-save-bar"><div><span>本次新增 {draftEntries.length} 項</span><strong>{draftTotals.kcal} kcal · 蛋白質 {draftTotals.protein.toFixed(1)} g</strong>{saveError && <small role="alert">儲存失敗，草稿仍保留</small>}</div><div><button type="button" disabled={!draftEntries.length || saving} aria-expanded={draftExpanded} onClick={() => setDraftExpanded((value) => !value)}>{draftExpanded ? '收合草稿' : '查看草稿'}</button><button type="button" className="primary" disabled={!draftEntries.length || saving} onClick={() => void saveDraft()}>{saving ? '儲存中…' : `儲存 ${draftEntries.length} 項`}</button></div></footer>
      {saving && <div className="v6-saving-lock" role="status" aria-live="polite"><span>正在儲存餐點…</span></div>}
    </section>
  </div>, document.body)
}
