...
export function RecordPage({ date, log, foods, onDate, onChange, onSaveFood, onDeleteFood }: {
  ...
  return <section className="page record-page">
    ...
    <Section title="睡眠" missing={log.sleepHours == null}><NumberField label="睡眠時間" value={log.sleepHours} unit="小時" step={0.25} onChange={(sleepHours) => onChange({ sleepHours })} /><SelectScale label="睡眠品質" value={log.sleepQuality} onChange={(sleepQuality) => onChange({ sleepQuality: sleepQuality as DailyLog['sleepQuality'] })} /></Section>
