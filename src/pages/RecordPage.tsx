...
import { dailyDeficit, estimatedTDEE, nutritionCoverageDisplay, parseLocalDate, sleepDurationHours } from '../calculations'
...
const blankWorkout = (): WorkoutEntry => ({ id: crypto.randomUUID(), type: 'walk', title: '步行', durationMinutes: 0, source: 'manual', activityKcalMode: 'included_in_daily_total' })
type CopyIntent = {
  kind: 'meal'
  title: string
  sourceTitle: string
...
export function RecordPage({ date, log, logs, foods, settings, planVersion, online, aiEnabled, foodMetadata, initialStage, initialFoodIntent, saveState, onDate, onChange, onEnableAI, onAIRun, onCommitMetadata, onFoodIntentConsumed }: {
  ...
  // Energy/device data improves estimates but is optional so people without a
  // wearable can still finish a tracking-only day.
  const eveningMissing = false
  const eveningMissingCount = 0
  const calorieMaximum = dailyTargets.calories.max
  const remainingCalories = dailyTargets.guidance.calories ? Math.max(0, calorieMaximum - (log.intakeKcal ?? 0)) : undefined
  const nutritionRows = [
  ...
  const copyRecentMeal = () => {
    const source = [...logs].filter((item) => item.date < date && item.mealDetails).sort((a, b) => b.date.localeCompare(a.date)).map((item) => {
      ...
      const meal = (['evening', 'dinner', 'lunch', 'breakfast'] as const).find((key) => mealDetails[key].some((line) => line.amount > 0))
      return meal ? { meal, details: mealDetails } : undefined
    }).find(Boolean)
    if (!source) return showCopyMessage('找不到最近可沿用的餐點')
    setCopyIntent({ kind: 'meal', title: '沿用最近一餐', sourceTitle: `最近${mealLabels[source.meal]}`, source: source.details[source.meal], meal: source.meal, toastLabel: '最近一餐' })
  }
  ...
  const stages: Array<{ id: RecordStage; label: string; sub: string; incomplete: boolean; complete: boolean; Icon: typeof Scale }> = [
    { id: 'morning', label: '早上', sub: '20 秒', incomplete: log.weightKg == null || log.sleepHours == null, complete: log.weightKg != null && log.sleepHours != null, Icon: Scale },
    { id: 'food', label: '飲食', sub: '隨吃隨記', incomplete: log.intakeKcal == null || log.proteinG == null || (dailyTargets.guidance.water && log.waterMl == null), complete: log.intakeKcal != null && log.proteinG != null && (!dailyTargets.guidance.water || log.waterMl != null), Icon: Utensils },
  ...
  return <section className="page record-page sprint-record">
    ...
    {activeStage === 'morning' && <div className="record-tab-panel">
      <div className="tab-intro"><strong>早上約 20 秒</strong><p>記錄體重、前一晚睡眠與排便；其他身體狀態可依需要填寫。</p></div>
      ...
      <Section title="顯示進階欄位" open={false}>
        ...
        <SelectScale label="活動相關不適（選填；0 無、5 很明顯）" min={0} max={5} value={log.lowerLegTightness} onChange={(lowerLegTightness) => onChange({ lowerLegTightness: lowerLegTightness as DailyLog['lowerLegTightness'] })} />
        {(log.lowerLegTightness ?? 0) >= 2 && <p className="recovery-inline">避免會加重不適的活動；3 以上以休息或不痛的低強度活動為主。</p>}
    ...
    {activeStage === 'food' && <div className="record-tab-panel">
      ...
      <section className="v6-water-row" aria-labelledby="water-row-title"><div><span id="water-row-title">白開水</span><strong>{Math.round(log.waterMl ?? 0).toLocaleString('zh-TW')}{dailyTargets.guidance.water ? `／${dailyTargets.waterTargetMl.toLocaleString('zh-TW')}` : ''} ml</strong></div><div><button type="button" onClick={() => onChange({ waterMl: (log.waterMl ?? 0) + 250 })}>+250</button><button type="button" onClick={() => onChange({ waterMl: (log.waterMl ?? 0) + 500 })}>+500</button><label><span className="sr-only">輸入白開水</span><input aria-label="輸入白開水" type="number" min="0" step="250" inputMode="numeric" value={log.waterMl ?? ''} placeholder="輸入" onChange={(event) => onChange({ waterMl: event.target.value === '' ? undefined : Math.max(0, Number(event.target.value)) })} /></label></div></section>
      <Section title="快速沿用"><div className="copy-actions"><button type="button" onClick={copyYesterdayBreakfast}><Copy />昨天早餐</button><button type="button" onClick={copyYesterdayFood}><Copy />昨天整天</button><button type="button" onClick={copyRecentMeal}><Copy />最近一餐</button></div></Section>
    ...
    {activeStage === 'evening' && <div className="record-tab-panel">
      ...
      <Section title="運動明細" open={false}>
        ...
        {showWorkoutForm && <div className="workout-form panel-inner"><div className="form-title"><strong>{editingWorkoutId ? '編輯運動' : '新增運動'}</strong><span>普通紀錄只需類型與時長</span></div><label className="select-field">類型<select value={workoutDraft.type} onChange={(event) => { const type = event.target.value as WorkoutType; setWorkoutDraft({ ...workoutDraft, type, title: workoutLabels[type] }) }}>{Object.entries(workoutLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><NumberField label="時長" value={workoutDraft.durationMinutes} unit="分" onChange={(durationMinutes) => setWorkoutDraft({ ...workoutDraft, durationMinutes: durationMinutes ?? 0 })} />
          ...
          <details className="manual-override"><summary>手動活動熱量覆寫</summary><p id="activity-kcal-mode-label">只有確定這筆活動不在每日活動能量摘要中，才可額外加入。</p><div className="segmented" role="group" aria-labelledby="activity-kcal-mode-label"><button type="button" aria-pressed={workoutDraft.activityKcalMode !== 'add_to_daily_total'} className={workoutDraft.activityKcalMode !== 'add_to_daily_total' ? 'selected' : ''} onClick={() => setWorkoutDraft({ ...workoutDraft, activityKcalMode: 'included_in_daily_total' })}>預設：不再加總</button><button type="button" aria-pressed={workoutDraft.activityKcalMode === 'add_to_daily_total'} className={workoutDraft.activityKcalMode === 'add_to_daily_total' ? 'selected' : ''} onClick={() => setWorkoutDraft({ ...workoutDraft, activityKcalMode: 'add_to_daily_total' })}>確定未包含，額外加入</button></div></details>
      ...
      <div className={`finalize-card standard-card ${log.dayFinalized ? 'done' : ''}`}><div><span>{log.dayFinalized ? '今天已完成' : log.needsRefinalization ? '資料已更新' : '晚間結算'}</span><strong>{log.dayFinalized ? finalDeficit == null ? '今日紀錄已結算' : `最終推估赤字 ${Math.round(finalDeficit)} kcal` : eveningMissing ? `尚缺 ${eveningMissingCount} 項資料` : log.needsRefinalization ? '重新完成結算' : '可以完成今日結算'}</strong><p>{log.dayFinalized ? finalTdee == null ? '活動或能量資料未填完整，因此不顯示消耗與赤字估算。' : `推估總消耗 ${Math.round(finalTdee)} kcal；裝置與熱量資料皆為估算。` : eveningMissing ? '補完後即可結算。' : '確認今天的資料後再完成。'}</p></div></div>
    </div>}
    ...
    {justFinalized && <div className="finalize-success" role="status"><Check /><strong>今日結算完成</strong><span>{estimatedTDEE(log) == null || dailyDeficit(log) == null ? '紀錄已儲存；補齊能量資料後可查看消耗與赤字估算。' : `攝取 ${Math.round(log.intakeKcal ?? 0)} · 消耗 ${Math.round(estimatedTDEE(log)!)} · 赤字 ${Math.round(dailyDeficit(log)!)} kcal`}</span></div>}
