...
export const makeBackup = (settings: ChallengeSettings, logs: DailyLog[], foods: CustomFood[], growth?: GrowthSnapshot): BackupPayload => ({
  schemaVersion: 1,
...
靜止能量摘要：${show(log.restingKcal, ' kcal')}
活動能量摘要：${show(log.activeKcal, ' kcal')}
...
特定運動快捷分鐘：${show(log.slowJogMinutes, ' 分')}
平均運動心率：${show(log.averageExerciseHeartRate, ' bpm')}
靜息心率：${show(log.restingHeartRate, ' bpm')}
HRV：${show(log.heartRateVariabilityMs, ' ms')}
運動明細：${log.workouts?.length ? log.workouts.map((workout) => `${workout.title} ${workout.durationMinutes}分${workout.distanceKm == null ? '' : ` ${workout.distanceKm}km`}${workout.activeKcal == null ? '' : ` ${workout.activeKcal}kcal`}（${workout.source === 'apple_watch' ? '穿戴裝置' : '手動'}；${workout.activityKcalMode === 'add_to_daily_total' ? '尚未包含，另行加計' : '已包含於摘要'}）`).join('；') : '—'}
...
身體不適程度：${show(log.lowerLegTightness, '/5')}
疼痛備註：${log.painNotes || '—'}
備註：${log.notes || '—'}`
  }).join('\n\n')
  ...
  const guidanceSummary = settings.guidanceMode === 'tracking_only'
    ? '追蹤模式：純記錄（尚未套用個人熱量、活動或營養目標）'
    : `基準體重：${settings.baselineWeightKg} kg\n目標體重：${settings.targetWeightKg} kg`
  return `【減脂追蹤摘要】
追蹤期間：${settings.startDate} 至 ${settings.finalWeighInDate}
${guidanceSummary}
目前為追蹤第 ${day} 天
...
截至日期預測體重：${prediction.confidence === 'insufficient' ? `資料不足（目前 ${prediction.sampleCount} 筆，需至少 7 筆晨間體重）` : `${prediction.value!.toFixed(1)} kg（${prediction.confidence === 'low' ? '低信心；' : '趨勢估算；'}約 ±0.5 kg 水分波動，預測不是保證）`}
資料完整率：${completeness}%（以追蹤開始至 ${reportEnd} 的預期天數計算）

分析提醒：活動合計只加上標記為「尚未包含」的運動；已包含於穿戴裝置／手動摘要的明細不重複計算。只有按下「完成今日結算」的日期會納入平均與累積赤字；日間資料不當作最終結果。此摘要是紀錄與估算，不是醫療診斷。`
}

export const buildCsv = (logs: DailyLog[]): string => {
  const headers = ['日期', '已結算', '結算時間', '需重新結算', '最終赤字kcal', '體重kg', '量測條件', '腰圍cm', '活動摘要kcal', '待同步運動kcal', '目前活動合計kcal', '靜止能量kcal', '攝取kcal', '蛋白質g', '碳水g', '脂肪g', '纖維g', '鈉mg', '白開水ml', '步數', '距離km', '運動分鐘', '運動明細數', '運動明細JSON', '平均心率', '靜息心率', 'HRVms', '睡眠開始', '睡眠結束', '睡眠小時', '排便', 'Bristol', '疲勞', '飢餓', '身體不適0至5', '不適備註', '備註']
