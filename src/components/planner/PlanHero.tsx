...
export function PlanHero({ today, log, plan, version, latestReview, profile, currentWeight, onOpenPlan, onOpenWeeklyReview }: { today: string; log: DailyLog; plan: FatLossPlan; version: PlanVersion; latestReview?: WeeklyReview; profile?: UserProfile; currentWeight?: number; onOpenPlan: () => void; onOpenWeeklyReview: () => void }) {
  ...
  return <>
    ...
    <button type="button" className="weekly-review-entry health-card" onClick={onOpenWeeklyReview}><span>本週檢討</span><strong>查看資料完整度與下週重點</strong><small>{latestReview ? `最近已檢討：${latestReview.weekStart} 至 ${latestReview.weekEnd}` : '本地規則先彙整，不會自動修改設定。'}</small></button>
  </>
}
