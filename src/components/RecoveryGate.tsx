...
export function RecoveryGate({ state, onContinueExisting, onImportBackup, onCreateNew }: RecoveryGateProps) {
  ...
  return <main className="onboarding v6-recovery-gate">
    ...
    {state.plannerDataUnavailable && <p className="v6-recovery-gate__warning" role="alert">長期計畫資料暫時無法確認；目前數量可能不完整，系統不會把此裝置當成全新裝置。</p>}

    <dl className="panel v6-recovery-gate__counts" aria-label="目前偵測到的資料">
      <div><dt>每日紀錄</dt><dd>{state.counts.dailyLogs} 筆</dd></div>
      <div><dt>餐點項目</dt><dd>{state.counts.mealLines} 項</dd></div>
      <div><dt>自訂食物</dt><dd>{state.counts.foods} 項</dd></div>
      <div><dt>長期計畫</dt><dd>{state.counts.plannerPlans} 個</dd></div>
      <div><dt>長期計畫相關資料</dt><dd>{state.counts.plannerRecords} 項</dd></div>
    </dl>
    ...
    <div className="v6-recovery-gate__actions">
      ...
      <button type="button" disabled={busy} onClick={() => importInput.current?.click()}>匯入追蹤與培育備份</button>
      {!confirmingNew
