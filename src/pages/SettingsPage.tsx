...
import type { GrowthSnapshot } from '../growth'
...
export function SettingsPage({ today, settings, reportSettings = settings, logs, foods, planner, onboardingIncomplete = false, plannerDataUnavailable = false, aiConfigured, online, onEnableAI, onWithdrawAI, onOpenPlanner, onOpenPlanHistory, onPlannerImport, onSettings, onImport, onClear, onExportCore, onSaveFood, onDeleteFood }: {
  today: string; settings: ChallengeSettings; logs: DailyLog[]; foods: CustomFood[]
  reportSettings?: ChallengeSettings
  ...
  onImport: (settings: ChallengeSettings, logs: DailyLog[], foods: CustomFood[], growth?: GrowthSnapshot) => Promise<void>
  ...
  const share = async () => {
    const text = buildWeeklySummary(reportSettings, logs, today)
    try {
      ...
      else downloadText(`fat-loss-tracker-summary-${today}.txt`, text)
    } catch (error) {
      if ((error as DOMException).name !== 'AbortError') { downloadText(`fat-loss-tracker-summary-${today}.txt`, text); setMessage('已改為下載 TXT。') }
    }
  }
  const exportReport = async (kind: 'png' | 'pdf') => {
    setGeneratingReport(kind); setMessage(kind === 'png' ? '正在產生分析圖卡…' : '正在產生 PDF…')
    try {
      const result = kind === 'png' ? await shareReportPng(reportSettings, logs, today) : await shareReportPdf(reportSettings, logs, today)
  ...
  const importFile = async (file?: File) => {
    ...
    try {
      ...
      await onImport(parsed.settings, parsed.logs, parsed.foods, parsed.growth); setMessage('匯入成功；匯入前的追蹤與培育資料已自動下載備份。')
  ...
  const importPlannerFile = async (file?: File) => {
    ...
    try {
      ...
      setMessage('長期計畫匯入成功；每日紀錄與基本設定未變更。')
  ...
  return <section className="page settings-page">
    ...
    <div className="privacy-card standard-card"><ShieldCheck aria-hidden="true" /><div><strong>{aiEnabled ? '日常資料留在裝置；AI 僅在你主動要求時連線' : '目前資料只儲存在此裝置'}</strong><p>{aiEnabled ? 'AI 只接收畫面列明的必要彙整資料；核心紀錄不會自動上傳。' : '刪除瀏覽器網站資料或移除 App 前，請先匯出 JSON 備份。'}</p></div></div>

    <div className="settings-group planner-settings-card health-card"><h3>個人減脂計畫</h3><p className="group-intro">回答生活型態與安全問卷後，產生可逐欄確認的熱量、活動與營養目標。AI 只協助分析草稿。</p>{plannerDataUnavailable && <p className="planner-load-warning" role="alert">長期計畫資料暫時無法讀取；為避免覆寫既有計畫，重新載入成功前已停用寫入與匯入。</p>}<div className="action-list"><button className="primary" disabled={plannerDataUnavailable} onClick={onOpenPlanner}>{planner.plans.length ? '查看目前計畫' : '填問卷並建立計畫'}</button><button disabled={plannerDataUnavailable} onClick={onOpenPlanHistory}>計畫歷史</button><button disabled={plannerDataUnavailable || !planner.plans.length} onClick={plannerBackup}>匯出長期計畫備份</button><button disabled={plannerDataUnavailable} onClick={() => plannerFileRef.current?.click()}>匯入長期計畫備份</button><input ref={plannerFileRef} hidden type="file" accept="application/json,.json" onChange={(event) => void importPlannerFile(event.target.files?.[0])} /></div></div>
    ...
    <details className="settings-details standard-card"><summary>舊版／手動追蹤設定</summary><div className="details-body"><p className="group-intro">追蹤期間為 {settings.startDate}～{settings.finalWeighInDate}。建立個人計畫後，首頁優先使用目前生效的計畫版本。</p><label className="setting-row"><span>開始追蹤日期</span><input type="date" value={settings.startDate} onChange={(event) => set('startDate', event.target.value)} /></label><label className="setting-row"><span>階段檢視日期</span><input type="date" value={settings.finalWeighInDate} onChange={(event) => set('finalWeighInDate', event.target.value)} /></label><SettingNumber label="基準體重" value={settings.baselineWeightKg} unit="kg" step={0.1} onChange={(value) => set('baselineWeightKg', value)} /><SettingNumber label="參考目標體重" value={settings.targetWeightKg} unit="kg" step={0.1} onChange={(value) => set('targetWeightKg', value)} /></div></details>
    {settings.guidanceMode === 'tracking_only'
      ? <div className="settings-group standard-card"><h3>目前是純記錄模式</h3><p className="group-intro">尚未套用個人熱量、活動、營養或步數目標，因此不顯示無意義的 0 目標。若需要建議值，請由上方安全問卷建立個人計畫。</p></div>
      : <>
        <details className="settings-details standard-card"><summary>手動健康目標（進階）</summary><div className="details-body"><p className="group-intro">這些是相容舊紀錄的手動值。一般使用者建議由安全問卷建立個人計畫。</p><SettingNumber label="攝取下限" value={settings.intakeKcalMinimum} unit="kcal" onChange={(value) => set('intakeKcalMinimum', value)} /><SettingNumber label="攝取上限" value={settings.intakeKcalMaximum} unit="kcal" onChange={(value) => set('intakeKcalMaximum', value)} /><SettingNumber label="活動目標" value={settings.activeKcalTarget} unit="kcal" onChange={(value) => set('activeKcalTarget', value)} /><SettingNumber label="蛋白質至少" value={settings.proteinMinimumG} unit="g" onChange={(value) => set('proteinMinimumG', value)} /><SettingNumber label="喝水至少" value={settings.waterMinimumMl} unit="ml" onChange={(value) => set('waterMinimumMl', value)} /><SettingNumber label="睡眠至少" value={settings.sleepMinimumHours} unit="小時" step={0.25} onChange={(value) => set('sleepMinimumHours', value)} /></div></details>
    ...
    {destructive?.kind === 'withdraw' && <DestructiveActionSheet title={destructive.clearRuns ? '撤回 AI 同意並清除執行紀錄' : '撤回 AI 同意'} description="撤回後不會再發出 AI 請求；這不會刪除每日紀錄或長期計畫。" deleteItems={destructive.clearRuns ? ['AI 資料分享同意', '本機 AI 執行紀錄'] : ['AI 資料分享同意']} preserveItems={['每日紀錄', '基本設定', '長期計畫', '食物紀錄']} confirmationPhrase="撤回" confirmLabel="確認撤回" onClose={() => setDestructive(undefined)} onConfirm={() => withdrawAI(destructive.clearRuns)} />}
  </section>
}
